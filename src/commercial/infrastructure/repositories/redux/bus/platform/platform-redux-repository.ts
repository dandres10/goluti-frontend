import { ACTIONS_PLATFORM } from './platform.action';
...
export class PlatformReduxRepository extends IPlatformReduxRepository {
    ...
    public updatePlatform(
        ...
        if (config?.dispatch) {
            config.dispatch(ACTIONS_PLATFORM.updatePlatformAction(params));
        }
    };

    public readUser(config: IConfigDTO): IUserReduxDTO | undefined {
        if (config?.selector) {
            const user = config.selector((state: any) => state?.commercial?.platform?.configuration?.user);
            return user;
        }
        return undefined;
    }
